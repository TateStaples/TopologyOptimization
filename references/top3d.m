...
% nelx = x len of solid, nely= y length of solid, nelz= z length of solid,
% volfrac=(1-porosity), penal= penalization power,rmin=filter size

% USER-DEFINED LOOP PARAMETERS
maxloop = 200;    % Maximum number of iterations
tolx = 0.01;      % Terminarion criterion - min improvement
displayflag = 0;  % Display structure flag (not sure how this works
...
% this is a vector of forces at the end x distributed along 0 to max z
[il,jl,kl] = meshgrid(0:nelx,nely,0:nelz); % this should be distributed
% forces along the top face
[il,jl,kl] = meshgrid(nelx, 0, 0:nelz);                 % Coordinates
loadnid = kl*(nelx+1)*(nely+1)+il*(nely+1)+(nely+1-jl); % Node IDs
loaddof = 3*loadnid(:) - 1;                             % DOFs
% USER-DEFINED SUPPORT FIXED DOFs - where is affixed (the base)

% the supports are x=0 along the surface by iterating thorugh y&z
[iif,jf,kf] = meshgrid(0:nelz,0,0:nelz); % this should be a base support
...
nele = nelx*nely*nelz;  % number of total nodds
ndof = 3*(nelx+1)*(nely+1)*(nelz+1); % number of degrees of freedom
F = sparse(loaddof,1,-1,ndof,1); % force
U = zeros(ndof,1); % displacement
freedofs = setdiff(1:ndof,fixeddof);  % where the non-fixed DOFs
KE = lk_H8(nu); % the stiffness matrix from the other function
nodegrd = reshape(1:(nely+1)*(nelx+1),nely+1,nelx+1);
nodeids = reshape(nodegrd(1:end-1,1:end-1),nely*nelx,1);
nodeidz = 0:(nely+1)*(nelx+1):(nelz-1)*(nely+1)*(nelx+1);
nodeids = repmat(nodeids,size(nodeidz))+repmat(nodeidz,size(nodeids));
edofVec = 3*nodeids(:)+1;
relElement = repmat([0 1 2 3*nely + [3 4 5 0 1 2] -3 -2 -1 ...
    3*(nely+1)*(nelx+1)+[0 1 2 3*nely + [3 4 5 0 1 2] -3 -2 -1]],nele,1);
edofMat = repmat(edofVec,1,24)+ relElement;
...
while change > tolx && loop < maxloop
    ...
    U(freedofs,:) = K(freedofs,freedofs)\F(freedofs,:); % displacement
...
K1 = [k(1) k(2) k(2) k(3) k(5) k(5);
    ...
    K1 = [k(1) k(2) k(2) k(3) k(5) k(5);k(2) k(1) k(2) k(4) k(6) k(7);k(2) k(2) k(1) k(4) k(7) k(6);k(3) k(4) k(4) k(1) k(8) k(8);k(5) k(6) k(7) k(8) k(1) k(2);k(5) k(7) k(6) k(8) k(2) k(1)];
