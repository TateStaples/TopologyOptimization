...
def main(x_nodes: int, y_nodes: int, z_nodes: int, volfrac: float, penal: float, rmin: float):
    ...
    # BC's and support
    dofs = np.arange(num_dofs)
    forces = get_load(shape)
    ...
    smooth_filter = Filter(rmin, shape)
    ...
    for loop in range(2000):
        ...
        print("it.: {0} , obj.: {1:.3f} Vol.: {2:.3f}, ch.: {3:.3f}".format(loop, obj, (g + volfrac * x_nodes * y_nodes) / (x_nodes * y_nodes), change))
    display_3d(x, shape)
